...
def _normalize_product(item: dict[str, Any]) -> ProductData:
    return ProductData(
        producto_id=str(item.get("producto_id", "") or ""),
        nombre=str(item.get("producto_nombre", "") or ""),
        categoria=str(item.get("categoria_nombre", "") or ""),
...
def _normalize_products(items: list[dict[str, Any]]) -> list[ProductData]:
    return [_normalize_product(item) for item in items]
...
def _normalize_categories(items: list[dict[str, Any]]) -> list[str]:
    ...
    seen: set[str] = set()
    for item in items:
        name = str(item.get("categoria_nombre", "") or "").strip()
...
def load_context(db_path: str) -> BusinessDataContext:
    """Carga datos desde Nexar Comercio y los adapta al contrato común.

    La lectura read-only y la tolerancia a esquemas variables siguen estando
    centralizadas en `services.nexar_importer`.
    """
